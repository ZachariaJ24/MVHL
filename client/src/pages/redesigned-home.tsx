      ...
      <section className="py-16 bg-slate-800/30">
        <div className="container">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-3xl font-bold text-white">Upcoming MVHL Matches</h2>
            <Link href="/matches">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {upcomingMatches.length > 0 ? upcomingMatches.map((match: any, index: number) => (
              <Card key={index} className="bg-slate-800/50 border-slate-700">
                <CardHeader className="text-center">
                  <div className="text-sm text-slate-400">{new Date().toLocaleDateString()}</div>
                  <CardTitle className="text-white">{match.homeTeam || "Team A"} vs {match.awayTeam || "Team B"}</CardTitle>
                </CardHeader>
              ...
              <div className="col-span-3 text-center py-12">
                ...
                <p className="text-slate-400">Check back later for the latest game schedule</p>
              </div>
            )}
          </div>
        </div>
            ...
            {recentTrades.length > 0 ? recentTrades.map((trade: any, index: number) => (
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      <div className="text-center">
                        <div className="text-sm font-medium text-white">{trade.fromTeam || "Ottawa Senators"}</div>
                        <div className="text-xs text-slate-400">Traded</div>
                        <div className="text-sm text-blue-400">{trade.offeredPlayer || "Xswickedpx"}</div>
                        <div className="text-xs text-slate-400">$1.50M</div>
                      </div>
                      <div className="text-2xl text-slate-500">⇄</div>
                      <div className="text-center">
                        <div className="text-sm font-medium text-white">{trade.toTeam || "Baltimore Bandits"}</div>
                        <div className="text-xs text-slate-400">Traded</div>
                        <div className="text-sm text-green-400">{trade.requestedPlayer || "Jayyroww"}</div>
                        <div className="text-xs text-slate-400">$0.75M</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm text-slate-400">
                        {trade.createdAt ? new Date(trade.createdAt).toLocaleDateString() : "1 day ago"}
                      </div>
              ...
              <Card className="bg-slate-800/50 border-slate-700">
                <CardContent className="p-12 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-white mb-2">No Recent Trades</h3>
                  <p className="text-slate-400">Trade activity will appear here when available</p>
                </CardContent>
              </Card>
            )}
          </div>
                    ...
                    {topStandings.length > 0 ? topStandings.map((team: any, index: number) => (
                      <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-slate-700/30">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <div className="font-medium text-white">{team.city} {team.name}</div>
                            <div className="text-sm text-slate-400">{team.division}</div>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="font-bold text-white">{team.points || 0} PTS</div>
                          <div className="text-sm text-slate-400">{team.wins || 0}W-{team.losses || 0}L-{team.otLosses || 0}OT</div>
                        </div>
                      </div>
                    )) : (
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    {upcomingMatches.length > 0 ? upcomingMatches.map((match: any, index: number) => (
                      <div key={index} className="text-center p-4 rounded-lg bg-slate-700/30">
                        <div className="font-medium text-white text-lg">{match.homeTeam || "Team A"} vs {match.awayTeam || "Team B"}</div>
                      ...
                      <div className="col-span-3 text-center py-8">
                        ...
                        <p className="text-slate-400">Recent results will appear here after games are completed</p>
                      </div>
                    )}
                  </div>
                </CardContent>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {latestNews.length > 0 ? latestNews.map((article: any, index: number) => (
              ...
              <div className="col-span-3 text-center py-12">
                ...
                <p className="text-slate-400">Check back soon for the latest league updates</p>
              </div>
            )}
          </div>
        </div>
